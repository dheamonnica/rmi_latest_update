              ...
              <table class="table table shopping-cart-item-table" id="table{{ $cart->id }}">
                ...
                <tbody>
                  @foreach ($cart->inventories as $item)
                    @php
                      $unit_price = $cart->auction_bid_id ? get_formated_value($cart->bid->amount_in_system_currency) : get_formated_value($item->current_sale_price());
