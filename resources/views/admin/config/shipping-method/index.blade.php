                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-5">
                                    @if (File::exists($logo_path))
                                        <img src="{{ asset($logo_path) }}" width="100" height="25"
                                            alt="{{ $type }}">
                                        <span class="spacer10"></span>
                                    @else
                                        <p class="lead">{{ $type }}</p>
                                    ...
                                    @foreach ($shipping_providers as $shipping_provider)
                                        ...
                                        <ul class="list-group">
                                            <li class="list-group-item">
                                                @if (File::exists($logo_path))
                                                    <img src="{{ asset($logo_path) }}" class="open-img-md"
                                                        alt="{{ $type }}">
                                                @else
                                                    ...
                                                    @if ($can_update)
                                                        ...
                                                        @if ($shipping_provider->code == 'shippo')
                                                            <a  href="javascript:void(0)"
                                                                data-link="{{ route('admin.setting.shippingMethod.activate', $shipping_provider->id) }}"
                                                                class="btn ajax-modal-btn btn-info">{{ trans('app.update') }}</a>
                                                        @else
                                                            <a href="javascript:void(0)"
                                                                data-link="{{ route('admin.setting.shippingMethod.activate', $shipping_provider->id) }}"
                                                                class="btn ajax-modal-btn btn-info">{{ trans('app.update') }}</a>
                                                ...
                                                @else
                                                    @if ($can_update)
                                                        <a href="javascript:void(0)"
                                                            data-link="{{ route('admin.setting.shippingMethod.activate', $shipping_provider->id) }}"
                                                            class="btn ajax-modal-btn btn-primary">{{ $has_config ? trans('app.reactivate') : trans('app.activate') }}</a>
                                                    @else
                                                        <span
                                                            class="label label-default">{{ trans('app.inactive') }}</span>
                                                    @endif
                                                @endif
