@if (!$offering->approved_by && (new \App\Helpers\Authorize(Auth::user(), 'approve_offering'))->check())
    {!! Form::open(['route' => ['admin.offering.setApprove', $offering], 'method' => 'put', 'class' => 'inline']) !!}
    <a href="javascript:void(0)"><i class="confirm ajax-silent fa fa-check"></i></a>
    {!! Form::close() !!}
@endif

@if ((new \App\Helpers\Authorize(Auth::user(), 'edit_offering'))->check())
...
@if ((new \App\Helpers\Authorize(Auth::user(), 'delete_offering'))->check())
    {!! Form::open([
