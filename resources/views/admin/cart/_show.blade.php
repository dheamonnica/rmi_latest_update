        ...
        <div class="tab-content">
                ...
                @if (count($cart->inventories) > 0)
                  @foreach ($cart->inventories as $item)
                    <tr>
                      ...
                      <td>{{ get_formated_currency($item->pivot->unit_price, 2, config('system_settings.currency.id')) }}</td>
                      <td>{{ get_formated_currency($item->pivot->quantity * $item->pivot->unit_price, 2, config('system_settings.currency.id')) }}</td>
                    </tr>
                  @endforeach
                @else
                  <tr id='empty-cart'>
                    <td colspan="5">{{ trans('help.empty_cart') }}</td>
                  </tr>
                @endif
              </tbody>
            </table>
          ...
          <div class="tab-pane" id="info_tab">
            <table class="table no-border">
              ...
              @if ($cart->bid)
                <tr>
                  <th class="text-right">{{ trans('auction::lang.bid_wins_at') }}:</th>
                  <td style="width: 75%;">
                    <small class="indent20"> {{ optional($cart->bid->accepted_at)->toDayDateTimeString() }} </small>
                  </td>
                </tr>
              @endif
          ...
          <div class="tab-pane" id="invoice_tab">
            <table class="table no-border">
              <tr>
                ...
                <td style="width: 75%;"><span class="lead"> {{ get_formated_currency($cart->grand_total, 2, config('system_settings.currency.id')) }}</span>
                </td>
              </tr>
              <tr>
                <th class="text-right">{{ trans('app.total') }}:</th>
                <td style="width: 75%;">{{ get_formated_currency($cart->total, 2, config('system_settings.currency.id')) }}</td>
              </tr>
              <tr>
                <th class="text-right">{{ trans('app.discount') }}:</th>
                <td style="width: 75%;"> - {{ get_formated_currency($cart->discount, 2, config('system_settings.currency.id')) }}</td>
              </tr>
              <tr>
                <th class="text-right">{{ trans('app.shipping') }}:</th>
                <td style="width: 75%;">{{ get_formated_currency($cart->shipping, 2, config('system_settings.currency.id')) }}</td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td style="width: 75%;">{{ get_formated_currency($cart->taxes, 2, config('system_settings.currency.id')) }}</td>
              </tr>
              <tr>
