@if ($budget->status === 0 && (new \App\Helpers\Authorize(Auth::user(), 'approve_budget'))->check())
    ...
    {!! Form::close() !!}
@endif
@if ((new \App\Helpers\Authorize(Auth::user(), 'edit_budget'))->check())
    <a href="javascript:void(0)" data-link="{{ route('admin.budget.edit', $budget->id) }}" class="ajax-modal-btn"><i
            data-toggle="tooltip" data-placement="top" title="{{ trans('app.edit') }}" class="fa fa-edit"></i></a>&nbsp;
@endif

@if ((new \App\Helpers\Authorize(Auth::user(), 'delete_budget'))->check())
    {!! Form::open([
